...
class PushListNodeTest: public ::testing::Test {
protected:
	t_list	list;
...
TEST_F(PushListNodeTest, empty)
{
	char	*data = strdup("hello");

	ASSERT_EQ(push_list_node(data, &list), 0);
...
TEST_F(PushListNodeTest, one_exist)
{
	char	*data = strdup("hello");
...
TEST_F(PushListNodeTest, multi_exist)
{
	char	*data = strdup("hello");
...
TEST_F(PushListNodeTest, multi_push)
{
	const char	*strs[] = {
		"aa",
		"bb",
	...
	for (int i=0; i < len; i++) {
		ASSERT_EQ(push_list_node(data[i], &list), 0);
	}
